...
The final thesis artifacts include a controlled apple restoration comparison using the same moved object and source-hole mask. In that comparison, LaMa gives the lowest measured source-hole color discontinuity and the cleanest visual result among the tested methods. This supports the selected restoration method for the apple case, but it does not solve restoration generally. Large disocclusions, structured backgrounds, shadows, reflections, and repeated textures remain difficult regardless of the selected restorer.
...
The evaluation reports quantitative metrics for the apple ablation, additional teapot and tree runs, a compact hyperparameter study, and a restoration comparison. It is still not equivalent to a benchmark evaluation and cannot establish broad statistical reliability, average improvement, or superiority over competing methods. More rigorous evaluation requires repeated trials across object categories, motion magnitudes, mask qualities, backgrounds, random seeds, prompts, and broader combinations of RAFT iterations, guidance weights, and DDIM schedules.

The reported apple ablation metrics are useful for the central RAFT question because they compare no-RAFT and RAFT-guided diffusion under fixed seeds. The hyperparameter study shows parameter sensitivity, especially for RAFT iteration count, but it is one-factor-at-a-time and uses one seed. The teapot and tree metrics are useful as additional inspectable cases, but they should not be interpreted as proof of broad generalization.
...
Finally, inference speed remains a practical limitation. The current system uses diffusion sampling, optical-flow guidance, and restoration, which can be expensive. The compact hyperparameter study suggests that fewer DDIM steps can reduce runtime substantially, but this comes with worse preservation in the apple case. Faster samplers, fewer refinement steps, model distillation, and asynchronous job scheduling would make the system more suitable for interactive use.

\section{Summary}

The correct scope is an implemented hybrid editing workflow with direct primitive controls, a focused fixed-seed apple ablation, two additional generated case studies, a compact hyperparameter sensitivity study, and a controlled restoration comparison. The evidence shows RAFT-guided improvement in pre-composite diffusion alignment, while final visual quality depends heavily on restoration and compositing. Stronger restoration, reliable non-rigid editing, broader quantitative testing, and faster interaction remain open work.
